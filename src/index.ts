...
class PolicyNetwork {
  model: tf.Sequential;
  currentActions_: Float32Array;

  /**
   * Constructor of PolicyNetwork.
   *
   * @param {number | number[] | tf.Sequential } hiddenLayerSizes
   *   Can be any of the following
   *   - Size of the hidden layer, as a single number (for a single hidden
   *     layer)
   *   - An Array of numbers (for any number of hidden layers).
   *   - An instance of tf.Sequential.
   */
  constructor(args: number | number[] | tf.Sequential ) {
    if (args instanceof tf.Sequential) {
      this.model = args;
    } else {
      this.createModel(args);
    }
  }
   ...
   */
  createModel(hiddenLayerSizes: number | number[] ) {
   ...
   *
   * @param {ISystem} system The system object to use during
   *   training.
   ...
   */
  async train(system: ISystem, optimizer: any, discountRate: number, numGames: number, maxStepsPerGame: number) {
    const allGradients:any = {};
    ...
    for (let i = 0; i < numGames; ++i) {
      ...
      // of every game.
      system.randomize();
      const gameRewards:any = [];
      const gameGradients:any = [];
      for (let j = 0; j < maxStepsPerGame; ++j) {
        ...
        const gradients = tf.tidy(() => {
          const inputTensor = system.getInputs();
          return this.getGradientsAndSaveActions(inputTensor).grads;
        });

        this.pushGradients(gameGradients, gradients);
        const action = this.currentActions_[0];
        const isDone = system.update(action);
  ...
  getGradientsAndSaveActions(inputTensor: tf.Tensor) {
    const f = () => tf.tidy(() => {
      const [logits, actions] = this.getLogitsAndActions(inputTensor);
      this.currentActions_ = actions.dataSync() as Float32Array;
   ...
   */
  getLogitsAndActions(inputs:tf.Tensor):any {
    return tf.tidy(() => {
      const logits = this.model.predict(inputs) as any;
   ...
   *   `batchSize`.
   */
  getActions(inputs:tf.Tensor):Float32Array {
   ...
   */
  pushGradients(record:{[varName: string]: tf.Tensor[]}, gradients:any) {
...
const MODEL_SAVE_PATH_ = 'indexeddb://tfjs-learning-example';
...
export class SaveablePolicyNetwork extends PolicyNetwork {
   ...
   *
   * @param {number | number[] | tf.Sequential} args
   */
  constructor(args: number | number[] | tf.Sequential) {
    super(args);
  }
   ...
   */
  static async loadModel(): Promise<SaveablePolicyNetwork> {
    const modelsInfo = await tf.io.listModels();
    if (MODEL_SAVE_PATH_ in modelsInfo) {
      console.log(`Loading existing model...`);
      const model = await tf.loadModel(MODEL_SAVE_PATH_) as tf.Sequential;
  ...
  hiddenLayerSizes():number | number[] {
    ...
    for (let i = 0; i < this.model.layers.length - 1; ++i) {
      let layer = this.model.layers[i] as any;
 ...
 *   Array of tf.Tensor.
 */
function discountAndNormalizeRewards(rewardSequences: number[][], discountRate: number): tf.Tensor[] {
  return tf.tidy(() => {
    const discounted = [];
 ...
 *   for the variables.
 */
function scaleAndAverageGradients(allGradients:{[varName: string]: tf.Tensor[][]}, normalizedRewards: tf.Tensor[]) {
  return tf.tidy(() => {
    const gradients:any = {};
